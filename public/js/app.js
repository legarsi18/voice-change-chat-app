...
// ミュートボタン用SVGアイコン（currentColor でボタン文字色に追従）
...
function _isPWA() {
  return window.matchMedia('(display-mode: standalone)').matches
    || window.navigator.standalone === true; // iOS Safari の standalone 判定
}
...
function _leaveRoomCleanup() {
  ...
  _wakeLock = null;
  // MediaSession をリセット（ロック画面の通話表示を消す）
...
function renderLobby(app, roomId, token) {
  ...
  document.getElementById('joinBtn').addEventListener('click', async (e) => {
    ...
    try {
      ...
      // CFセッションIDはルーム画面遷移時に毎回取得するため、ここでは保存しない
      saveSession({ roomId, token, joinedAt: Date.now() });
...
async function renderRoom(app, roomId) {
  ...
  if (voiceChanger) {
    await voiceChanger.destroy().catch(() => {});
    voiceChanger = null;
  }
  ...
  app.innerHTML = `
    <div class="screen room-screen">
      ...
      <footer class="room-footer">
        ...
        <button class="btn btn-mute muted" id="muteBtn"></button>
  ...
  // 毎回新しいCFセッションを取得（古いセッションIDの再利用による406エラーを根本防止）
  let cfSessionId;
  {
    let joinStatus = 0;
    try {
      const joinRes = await fetch(`${WORKER_URL}/api/rooms/${roomId}/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!joinRes.ok) throw new Error(joinData.error || `接続エラー (${joinRes.status})`);
      if (!joinData.sessionId) throw new Error('セッションIDが取得できませんでした');
      cfSessionId = joinData.sessionId;
  ...
  // ─── MediaSession API: iOS/Android にオーディオアプリとして認識させる ───
  // iOS 16.4+ PWA では MediaSession + playbackState='playing' により
  // OS がバックグラウンドでも AudioContext を維持しやすくなる
  if ('mediaSession' in navigator) {
    ...
    // ロック画面の操作ボタン（停止のみ。誤操作防止のため何もしない）
    navigator.mediaSession.setActionHandler('stop', () => {});
    navigator.mediaSession.setActionHandler('pause', () => {});
  }

  // ─── PWA インストール案内バナー（未インストール時のみ表示） ───
  // iOS 16.4+ / Android での PWA 化がバックグラウンド音声の最善策
  if (!_isPWA() && !sessionStorage.getItem('pwaBannerDismissed')) {
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
    const isAndroid = /Android/.test(navigator.userAgent);
    if (isIOS || isAndroid) {
      ...
      banner.id = 'pwaBanner';
      const desc = isIOS
        ? 'Safari 下部の <b>共有ボタン</b> →「<b>ホーム画面に追加</b>」でインストールすると、別アプリに切り替えてもボイチェンで話し続けられます。'
        : '<b>ブラウザメニュー</b> →「<b>ホーム画面に追加</b>」または「<b>アプリをインストール</b>」でインストールすると、別アプリに切り替えてもボイチェンで話し続けられます。';
      banner.innerHTML = `
        <span class="pwa-banner-icon">📲</span>
        <div class="pwa-banner-body">
          <div class="pwa-banner-title">アプリとしてインストールすると通話品質が向上します</div>
          <div class="pwa-banner-desc">${desc}</div>
        </div>
        <button class="pwa-banner-close" id="pwaBannerClose" aria-label="閉じる">×</button>
      `;
  ...
  document.getElementById('muteBtn').innerHTML = muteBtnHTML(true);
  ...
  document.getElementById('muteBtn').addEventListener('click', () => {
    ...
    document.getElementById('muteBtn').innerHTML = muteBtnHTML(isMuted);
      ...
      if (!document.getElementById('disconnectToast')) {
        ...
        document.getElementById('reconnectBtn').addEventListener('click', () => {
          ...
          if (rid) {
            ...
            if (savedToken) {
              if (roomClient) { roomClient.onEvent = () => {}; roomClient.destroy(); roomClient = null; }
              voiceChanger?.destroy(); voiceChanger = null;
